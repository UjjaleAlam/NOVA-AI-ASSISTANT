"""
Executive Assistant Agent - Phase 34
High-level task management, scheduling, prioritization, delegation, decision support.
...
def init_ea_db():
    ...
    # Tasks (enhanced from context_engine)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_tasks (
            id TEXT PRIMARY KEY,
            title TEXT NOT NULL,
            description TEXT,
            status TEXT DEFAULT 'pending',  -- pending, in_progress, waiting, completed, cancelled, deferred
            priority INTEGER DEFAULT 3,     -- 1=critical, 2=high, 3=medium, 4=low, 5=someday
            category TEXT,                  -- work, personal, health, learning, admin, finance
            project_id TEXT,
            ...
            context TEXT,                   -- JSON for extra context
            energy_level INTEGER,           -- 1=low, 2=medium, 3=high energy required
            time_of_day TEXT,               -- morning, afternoon, evening, anytime
            location TEXT,                  -- office, home, anywhere, specific
            delegated_to TEXT,
            delegated_at REAL,
            reminder_at REAL,
            recurrence TEXT,                -- RRULE string
            source TEXT,                    -- manual, email, meeting, auto, suggestion
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_projects (
            ...
            category TEXT,
            goal_id TEXT,                   -- link to goal_awareness
            start_date REAL,
            target_date REAL,
            completed_at REAL,
            budget REAL,
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_calendar (
            ...
            event_type TEXT,               -- meeting, focus, break, travel, personal, appointment
            status TEXT DEFAULT 'confirmed', -- confirmed, tentative, cancelled
            location TEXT,
            attendees TEXT,                 -- JSON array
            meeting_link TEXT,
            recurrence TEXT,                -- RRULE
            reminders TEXT,                 -- JSON array of minutes before
            task_id TEXT,                   -- link to task
            project_id TEXT,
            calendar_source TEXT,           -- google, outlook, local, caldav
            created_at REAL NOT NULL,
            updated_at REAL NOT NULL
        )
    """)
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_meetings (
            ...
            start_time REAL NOT NULL,
            end_time REAL NOT NULL,
            attendees TEXT,                 -- JSON array of {name, email, role, required}
            organizer TEXT,
            location TEXT,
            meeting_link TEXT,
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_decisions (
            ...
            title TEXT NOT NULL,
            description TEXT,
            context TEXT,                   -- why this decision
            options TEXT,                   -- JSON array of {option, pros, cons, score}
            chosen_option TEXT,
            rationale TEXT,
            decision_type TEXT,             -- strategic, tactical, operational, hiring, budget, technical
            impact_scope TEXT,              -- personal, team, department, organization
            impact_level INTEGER,           -- 1=low, 2=medium, 3=high, 4=critical
            status TEXT DEFAULT 'pending',  -- pending, decided, implemented, reviewed, reversed
            decided_at REAL,
            decided_by TEXT,
            review_date REAL,
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_contacts (
            ...
            role TEXT,
            organization TEXT,
            relationship TEXT,             -- colleague, manager, direct_report, client, vendor, friend, family
            department TEXT,
            manager_id TEXT,               -- self-referential
            preferred_contact TEXT,        -- email, phone, slack, teams, in_person
            timezone TEXT,
            working_hours TEXT,            -- JSON {start, end, days}
            notes TEXT,
            tags TEXT,                      -- JSON array
            last_contact REAL,
            contact_frequency TEXT,        -- daily, weekly, monthly, quarterly, as_needed
            created_at REAL NOT NULL,
            updated_at REAL NOT NULL
        )
    """)
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_communications (
            ...
            channel TEXT,                  -- email, phone, slack, teams, in_person, sms
            subject TEXT,
            body TEXT,
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_plans (
            id TEXT PRIMARY KEY,
            plan_type TEXT,                -- daily, weekly, monthly, quarterly
            period_start REAL NOT NULL,
            period_end REAL NOT NULL,
            objectives TEXT,               -- JSON array
            priorities TEXT,               -- JSON array of {task_id, priority_rank}
            time_blocks TEXT,              -- JSON array
            focus_areas TEXT,              -- JSON array
            success_criteria TEXT,         -- JSON array
            review_notes TEXT,
            status TEXT DEFAULT 'draft',   -- draft, active, completed, reviewed
            created_at REAL NOT NULL,
            updated_at REAL NOT NULL
        )
    """)

    # Habits/Routines (enhanced)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_habits (
            id TEXT PRIMARY KEY,
            name TEXT NOT NULL,
            description TEXT,
            frequency TEXT,                -- daily, weekdays, weekends, weekly, monthly, custom
            target_count INTEGER DEFAULT 1,
            duration_minutes INTEGER,
            preferred_time TEXT,           -- morning, afternoon, evening, specific HH:MM
            days_of_week TEXT,             -- JSON array [0-6]
            category TEXT,                 -- health, productivity, learning, mindfulness, social
            streak_current INTEGER DEFAULT 0,
            streak_longest INTEGER DEFAULT 0,
            total_completions INTEGER DEFAULT 0,
            is_active BOOLEAN DEFAULT 1,
            trigger TEXT,                  -- what triggers this habit
            reward TEXT,
            difficulty INTEGER DEFAULT 3,  -- 1-5
            accountability TEXT,           -- partner, app, public, none
            created_at REAL NOT NULL,
            updated_at REAL NOT NULL
        )
    """)
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_inbox (
            ...
            title TEXT NOT NULL,
            description TEXT,
            source TEXT,                   -- voice, email, web, app, manual
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_focus_sessions (
            ...
            start_time REAL NOT NULL,
            end_time REAL,
            session_type TEXT,             -- pomodoro, deep_work, shallow_work, break
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_energy_logs (
            ...
            activity TEXT,                 -- what you were doing
            notes TEXT,
            created_at REAL NOT NULL
        )
    """)
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_delegations (
            id TEXT PRIMARY KEY,
            task_id TEXT,
            delegated_to TEXT,             -- contact_id or name
    ...
    # Quick capture / Voice memos
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ea_quick_capture (
            id TEXT PRIMARY KEY,
            content TEXT NOT NULL,
            content_type TEXT,             -- text, voice, image, file
            source TEXT,                   -- voice, widget, shortcut, email
            captured_at REAL NOT NULL,
            processed BOOLEAN DEFAULT 0,
            processed_at REAL,
            task_id TEXT,
...
def create_task(self, title: str, description: str = "", **kwargs) -> str:
    ...
    def update_task(self, task_id: str, **kwargs) -> bool:
        ...
        cursor.execute(f"UPDATE ea_tasks SET {', '.join(updates)} WHERE id = ?", params)
        conn.commit()
        conn.close()
        return True
...
class ProjectManager:
    ...
    def get_project(self, proj_id: str) -> Dict:
        conn = get_ea_connection()
        cursor = conn.cursor()
        ...
        if row:
            return {
                ...
                "goal_id": row[6], "start_date": row[7], "target_date": row[8],
...
class DecisionManager:
    ...
    def create_decision(self, title: str, description: str = "", **kwargs) -> str:
        dec_id = f"dec_{int(time.time() * 1000) % 100000000:08d}"
        conn = get_ea_connection()
        cursor = conn.cursor()
        cursor.execute("""
    ...
    def get_decision(self, dec_id: str) -> Dict:
        conn = get_ea_connection()
        cursor = conn.cursor()
        ...
        if row:
            return {
                ...
                "decision_type": row[7], "impact_scope": row[8],
                "impact_level": row[9], "status": row[10],
                "decided_at": row[11], "decided_by": row[12],
                "review_date": row[13], "related_tasks": json.loads(row[14]) if row[14] else [],
...
class ContactManager:
    ...
    def add_contact(self, name: str, **kwargs) -> str:
        contact_id = f"contact_{int(time.time() * 1000) % 100000000:08d}"
        conn = get_ea_connection()
        cursor = conn.cursor()
        cursor.execute("""
...
class Planner:
    ...
    def create_daily_plan(self, date: str = None, **kwargs) -> str:
        plan_id = f"plan_{int(time.time() * 1000) % 100000000:08d}"
        if date:
        ...
        cursor.execute("""
            INSERT INTO ea_plans (id, plan_type, period_start, period_end,
                                 ...
                                 created_at, updated_at)
            VALUES (?, 'daily', ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?)
        """, (f"plan_{int(time.time() * 1000) % 100000000:08d}",
    ...
    def create_weekly_plan(self, week_start: str = None, **kwargs) -> str:
...
class InboxManager:
    ...
    def capture(self, content: str, title: str = "", **kwargs) -> str:
        item_id = f"inbox_{int(time.time() * 1000) % 100000000:08d}"
        conn = get_ea_connection()
        cursor = conn.cursor()
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (f"inbox_{int(time.time() * 1000) % 100000000:08d}", title or content[:50],
              content, kwargs.get("source", "manual"), time.time(),
    ...
    def process_item(self, item_id: str, action: str, **kwargs) -> bool:
        """action: 'task', 'event', 'discard', 'delegate'"""
        conn = get_ea_connection()
        cursor = conn.cursor()
...
class QuickCapture:
    ...
    def capture(self, content: str, **kwargs) -> str:
        item_id = f"qc_{int(time.time() * 1000) % 100000000:08d}"
        conn = get_ea_connection()
        cursor = conn.cursor()
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (f"qc_{int(time.time() * 1000) % 100000000:08d}", content,
...
class DelegationManager:
    ...
    def delegate(self, task_id: str, delegated_to: str, **kwargs) -> str:
        del_id = f"del_{int(time.time() * 1000) % 100000000:08d}"
        conn = get_ea_connection()
        cursor = conn.cursor()
        cursor.execute("""
...
class FocusManager:
    ...
    def start_session(self, task_id: str = None, duration: int = 25,
        ...
        session_id = f"fs_{int(time.time() * 1000) % 100000000:08d}"
        conn = get_ea_connection()
        cursor = conn.cursor()
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (f"fs_{int(time.time() * 1000) % 100000000:08d}", task_id,
              duration, session_type, time.time(), time.time()))
        conn.commit()
        conn.close()
        return session_id
    ...
    def end_session(self, session_id: str, **kwargs) -> bool:
        ...
        cursor.execute("""
            ...
            WHERE id = ?
        """, (time.time(), kwargs.get("actual_duration"),
...
class EnergyTracker:
    ...
    def log_energy(self, energy: int, focus: int = None, mood: str = "",
        ...
        log_id = f"en_{int(time.time() * 1000) % 100000000:08d}"
        conn = get_ea_connection()
        cursor = conn.cursor()
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (f"en_{int(time.time() * 1000) % 100000000:08d}", time.time(),
              energy, focus, mood, activity, notes, time.time()))
        conn.commit()
        conn.close()
        return log_id
...
class HabitTracker:
    ...
    def add_habit(self, name: str, **kwargs) -> str:
        habit_id = f"habit_{int(time.time() * 1000) % 100000000:08d}"
        conn = get_ea_connection()
        cursor = conn.cursor()
        cursor.execute("""
    ...
    def complete_habit(self, habit_id: str, date: str = None, **kwargs) -> str:
        completion_id = f"hc_{int(time.time() * 1000) % 100000000:08d}"
...
class ExecutiveAssistant:
    ...
    def morning_briefing(self) -> str:
        ...
        briefing = f"Good morning! Here's your briefing for {datetime.now().strftime('%A, %B %d')}:\n\n"
        
        if dash["overdue_tasks"]:
            briefing += f"[WARNING]️ {dash['overdue_tasks']} overdue tasks need attention\n"
        if dash["today_tasks"]:
            briefing += f"📋 {dash['today_tasks']} tasks due today\n"
        if dash["today_events"]:
            briefing += f"📅 {dash['today_events']} events scheduled\n"
        if dash["upcoming_meetings"]:
            briefing += f"🤝 {dash['upcoming_meetings']} meetings in next 4 hours\n"
        if dash["inbox_count"]:
            briefing += f"📥 {dash['inbox_count']} items in inbox\n"
        if dash["habits_due"]:
            briefing += f"🔄 {dash['habits_due']} habits to complete\n"
        
        briefing += f"\n⚡ Energy: {dash['energy_trend']} ({dash['avg_energy']:.0f}/10)"
        ...
        if priorities:
            ...
            for i, t in enumerate(priorities, 1):
                briefing += f"\n  {i}. {t['title']} (due: {datetime.fromtimestamp(t['due_date']).strftime('%H:%M') if t.get('due_date') else 'no due date'})"
        
        return briefing

    def evening_review(self) -> str:
        """Generate evening review."""
        # Would analyze completed tasks, focus sessions, energy, etc.
